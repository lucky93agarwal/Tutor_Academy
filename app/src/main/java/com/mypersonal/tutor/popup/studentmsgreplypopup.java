...
public class studentmsgreplypopup {
    ...
    public studentmsgreplypopup(Context context) {
...
//        epicDialog.setCancelable(false);
    ...
    private void sendrating() {
        ...
        Log.d("StatusLucckyNew","requst  ="+new Gson().toJson(request));


        UserService service = ServiceGenerator.createService(UserService.class, null, null);
        service.sebd_message_send(request).enqueue(new Callback<RequestResponseJson>() {
            @Override
            public void onResponse(Call<RequestResponseJson> call, retrofit2.Response<RequestResponseJson> response) {
                Log.d("StatusLucckyNew","requst  ="+new Gson().toJson(response.body()));
