...
public class TutorEditProfileActivity extends AppCompatActivity {
    ...
    public EditText etname,etemail,etmobile,etage, etexpirence, etspecialization, etarea_ofteaching;
    public Spinner sp_quialifical, sp_tutor_fee_structure;
    ...
    public String spinnertutorqualification, spinnertutorfees;

    public Button btnsubject, btncity, btndays, btntutor_willing_to_traveling, btntutor_tuition_type, btntime_duration, btnupload_student_pic, btnsubmit;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        etage = (EditText)findViewById(R.id.ui_tutor_age);
        etarea_ofteaching = (EditText)findViewById(R.id.area_ofteaching);
        etexpirence = (EditText)findViewById(R.id.tutor_experience);
        etspecialization = (EditText)findViewById(R.id.specialization);
...
//                    startActivityForResult(photoPickerIntent, CAMERA_REQUEST);
                    ...
                    AlertDialog.Builder builder = new AlertDialog.Builder(TutorEditProfileActivity.this);
        ...
        etexpirence.setText(sharedPreferences.getString("tutor_exp",""));
        etspecialization.setText(sharedPreferences.getString("specialization",""));

        btnsubject.setText(sharedPreferences.getString("subject_offerd",""));
        btndays.setText(sharedPreferences.getString("day",""));
        btncity.setText(sharedPreferences.getString("Tutercurrent_city",""));
        ...
        btntime_duration.setText(sharedPreferences.getString("timeDuration",""));


        Glide.with(TutorEditProfileActivity.this).load(sharedPreferences.getString("image","")).placeholder(R.drawable.logo_light).into(ivstudent_pic);
...
//                onChangeSelectedReceivers();
            ...
            public void onResponse(Call<CitySignupResponseJson> call, Response<CitySignupResponseJson> response) {
                if (response.isSuccessful()) {
                    ...
                    if (response.body().status.equalsIgnoreCase("0")) { ////New App
                        ...
                        Toast.makeText(TutorEditProfileActivity.this, "Techincal problem. Please contact the admistrator", Toast.LENGTH_SHORT).show();
                    ...
                    switch (response.code()) {
                        case 401:
                            Toast.makeText(TutorEditProfileActivity.this, "email and pass not check", Toast.LENGTH_SHORT).show();
                            break;
                        case 403:
                            Toast.makeText(TutorEditProfileActivity.this, "ForbiddenException", Toast.LENGTH_SHORT).show();
                            break;
                        case 404:
                            Toast.makeText(TutorEditProfileActivity.this, "not found", Toast.LENGTH_SHORT).show();
                            break;
                        case 500:
                            Toast.makeText(TutorEditProfileActivity.this, "server broken", Toast.LENGTH_SHORT).show();
                            break;
                        default:
                            Toast.makeText(TutorEditProfileActivity.this, "unknown error", Toast.LENGTH_SHORT).show();
                            break;
                    }
                    Toast.makeText(TutorEditProfileActivity.this, "Something error please try again", Toast.LENGTH_SHORT).show();
    ...
    private void tutorSignup() {
        ...
        String tutor_age = etage.getText().toString().trim();
        String tutor_exp = etexpirence.getText().toString().trim();
        String subject_offerd = btnsubject.getText().toString().trim();
        String specializaionT = etspecialization.getText().toString().trim();
...
//        String imageString1 = student_pic.getImageMatrix().toString().trim();
      /*  int age = Integer.parseInt(tutor_age);
        ...
        Log.d("LuckyYUYU", "day = " + btndays.toString());
        Log.d("LuckyYUYU", "subject_offerd = " + btnsubject.getText().toString());
        Log.d("LuckyYUYU", "any specialization = " + etspecialization.getText().toString());
        ...
        edits.putString("age", etage.getText().toString().trim());
        edits.putString("qualification", spinnertutorqualification);
        edits.putString("tutor_exp", etexpirence.getText().toString().trim());
        edits.putString("Subjectid", subjectarray.toString());


        edits.putString("day", btndays.getText().toString());
        ...
        edits.putString("specialization", etspecialization.getText().toString().trim());
        edits.putString("mobile", etmobile.getText().toString().trim());
        edits.putString("age", etage.getText().toString().trim());
        edits.putString("qualification", spinnertutorqualification);
        ...
        Log.d("StatusLucckydfj", "age  =" + etage.getText().toString().trim());
...
//                        edits.apply();
                        ...
                        Intent intent = new Intent(TutorEditProfileActivity.this, TutorActivity.class);
