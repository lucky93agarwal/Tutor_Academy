...
import com.mypersonal.tutor.popup.studentmsgreplypopup;
...
public class MainActivity extends AppCompatActivity {
    ...
    private static studentmsgreplypopup addpopup;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        addpopup = new studentmsgreplypopup(MainActivity.this);
...
//        filterCourse();
    ...
    public void cancel(String status,String id){
        ...
        service.cancel_request_li_new(request).enqueue(new Callback<RequsetResponseRequestJson>() {
            @Override
            public void onResponse(Call<RequsetResponseRequestJson> call, retrofit2.Response<RequsetResponseRequestJson> response) {
                ...
                if (response.isSuccessful()) {
                    ...
                    if (response.body().status.equalsIgnoreCase("0")) {
                        Toast.makeText(MainActivity.this, "No Request Found", Toast.LENGTH_LONG).show();
                    } else {

                        Toast.makeText(MainActivity.this, "Request Send", Toast.LENGTH_LONG).show();
                    ...
                    switch (response.code()) {
                        case 401:
                            Toast.makeText(MainActivity.this, "email and pass not check", Toast.LENGTH_SHORT).show();
                            break;
                        case 403:
                            Toast.makeText(MainActivity.this, "ForbiddenException", Toast.LENGTH_SHORT).show();
                            break;
                        case 404:
    ...
    public void doAddition(String status,String id){
        ...
        Log.d("StatusLuccky","requst  ="+new Gson().toJson(request));
        UserService service = ServiceGenerator.createService(UserService.class, null, null);
        service.request_li_new(request).enqueue(new Callback<RequsetResponseRequestJson>() {
            @Override
            public void onResponse(Call<RequsetResponseRequestJson> call, retrofit2.Response<RequsetResponseRequestJson> response) {
                Log.d("StatusLuccky", "id Status =" + response.code());
                if (response.isSuccessful()) {
